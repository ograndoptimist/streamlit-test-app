...
def display_streamlit_updates():
    ...
    with st.expander("Streamlit 1.32 Announcement", expanded=False):
        ...
        st.image(img, caption="Streamlit 1.32 Announcement", use_column_width="auto", clamp=True, channels="RGB",
                 output_format="PNG")
        st.markdown(
            "For more details on this version, check out the [Streamlit Forum post](https://docs.streamlit.io/library/changelog#version-1320).")
...
def main():
    ...
    if not st.session_state.history:
        ...
        if isinstance(updates, dict):  # Check if updates is a dictionary
            ...
            st.session_state.conversation_history = [
                {"role": "system",
                 "content": "You are Streamly, a specialized AI assistant trained to assist with the logic and programming using Streamlit."},
                {"role": "system", "content": "Refer to conversation history to provide context to your reponse."},
                {"role": "system",
                 "content": "Use the streamlit_updates.json local file to look up the latest Streamlit feature updates."},
                {"role": "system",
                 "content": "When responding, provide code examples, links to documentation, and code examples from Streamlit API to help the user."},
